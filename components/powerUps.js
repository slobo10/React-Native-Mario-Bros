...
const Powerup = (props) => {
  ...
  var [scrollDistanceState, setScrollDistanceState] = useState(
    scrollDistance.current.current
  );
      ...
      case "mushroom": {
        return (
          <>
            <Circle
              cx={
                (currentPowerup.position[0] - scrollDistanceState + 0.5) *
                useContext(gameContext).game.gridSize
              }
              cy={
                useContext(gameContext).game.height -
                (currentPowerup.position[1] + 0.5) *
                  useContext(gameContext).game.gridSize
              }
              r={useContext(gameContext).game.gridSize / 2}
              fill="red"
            />
          </>
        );
