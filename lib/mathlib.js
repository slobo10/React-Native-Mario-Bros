const checkForCollision = (x, y, xSpeed, ySpeed, height, bricks, topAction, bottomAction, leftAction, rightAction) => {
    ...
    for (var i in bricks){
        if (bricks[i].type === undefined){
            continue;
        } else if (y >= bricks[i].posision[1] + 1 + ySpeed && y <= bricks[i].posision[1] + 1 && x > bricks[i].posision[0] - 1 && x < bricks[i].posision[0] + (bricks[i].dim !== undefined ? bricks[i].dim[0] : 1) && bricks[i].type !== 'hidden'){
    ...
    for (var i in bricks){
        if (bricks[i].type === 'hidden' || 'none'){
            continue;
        } else if (x >= bricks[i].posision[0] + (bricks[i].dim !== undefined ? bricks[i].dim[0] : 1) + xSpeed  && x <= bricks[i].posision[0] + (bricks[i].dim !== undefined ? bricks[i].dim[0] : 1) && newPosision[1] > bricks[i].posision[1] - (bricks[i].dim !== undefined ? bricks[i].dim[1] : 1) - height + 1 && newPosision[1] < bricks[i].posision[1] + 1){
