const checkForCollision = (x, y, xSpeed, ySpeed, height, bricks, topAction, bottomAction, leftAction, rightAction) => {
    ...
    for (var i in bricks){
        if (y >= bricks[i].posision[1] + 1 + ySpeed && y <= bricks[i].posision[1] + 1 && x > bricks[i].posision[0] - 1 && x < bricks[i].posision[0] + (bricks[i].dim !== undefined ? bricks[i].dim[0] : 1) && bricks[i].type !== 'hidden'){
